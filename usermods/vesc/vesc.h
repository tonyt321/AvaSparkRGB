 ...
 */
 uint16_t mode_batpercent(void) {
  if (batpercentage > 94){batpercentage = 100;}
...
unsigned long a_read_milisec = 0;  // analog read limit
unsigned long vesc_get_milisec = 0;  // analog read limit
...
int volt_to_percent(int volts) {
  ...
  void get_imu_data(){
    ...
    filteredx = rawx * .15 + (filteredx * (1.0 - .15));
    filteredy = rawy * .15 + (filteredy * (1.0 - .15));
    filteredz = rawz * .15 + (filteredz * (1.0 - .15));
...
//	DISABLED = 15
  ...
  if(fpkg_switchState == 2){footpad_press = 3;}
     ...
     if ((current < 5) && (rpm > -2 && rpm < 2) && ((dutycycle * 100 > -2) && (dutycycle * 100) < 2) && (imu_inactivity_count > 2)){
      if(active_milis_dim + 2000 < millis()){
      is_idle = true;
      }
      }else{
...
if ((a_read_milisec + 200) < millis()){    // limit loop to 5 times a sec
